...
class Runtime(TableWithDate):
    ...
    lang_problem = relationship(
        "LangProblem", back_populates="runtimes", primaryjoin="LangProblem.id==Runtime.lang_problem_id"
    )

    game_id = sa.Column(sa.Integer, sa.ForeignKey("games.id", ondelete="SET NULL"))
    game = relationship("Game", back_populates="runtimes", primaryjoin="Game.id==Runtime.game_id")
