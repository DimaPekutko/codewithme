...
class PlaygroundStorage:
    ...
    async def get_runtimes(self, lproblem_id: int, user_id: int, game_id: Optional[int]) -> List[schemas.FullRuntime]:
        async with async_session() as session:
            q = sa.select(models.Runtime).where(
                models.Runtime.lang_problem_id == lproblem_id, models.Runtime.user_id == user_id
            )
            if game_id:
    ...
    async def update_runtime(self, runtime_id: int, payload: schemas.RuntimeUpdatePayload):
        async with async_session() as session:
            q = sa.update(models.Runtime).where(models.Runtime.id == runtime_id).values(**payload.model_dump())
    ...
    async def get_games(self, user_id: int) -> schemas.FullGame:
        async with async_session() as session:
            q = sa.select(models.Game).where(sa.or_(models.Game.user1_id == user_id, models.Game.user2_id == user_id))
    ...
    async def create_game(self, lproblem_id: int, user1_id: int, user2_id: int, room_id: str) -> int:
        async with async_session() as session:
            game = models.Game(lang_problem_id=lproblem_id, user1_id=user1_id, user2_id=user2_id, room_uid=room_id)
            session.add(game)
            await session.commit()
            return game.id
    ...
    async def find_problem_by_params(self, payload: schemas.SearchGamePayload) -> int:
        async with async_session() as session:
            ...
            q = sa.select(model.id).where(
                model.status == problem_types.ProblemLangStatus.active,
            ...
            q = q.join(problem_models.Problem, problem_models.Problem.id == model.problem_id)

            q = q.join(
                problem_models.ProblemCatM2M, problem_models.Problem.id == problem_models.ProblemCatM2M.problem_id
            )

            q = q.join(
                problem_models.ProblemCategory,
    ...
    async def finish_game(self, game_id: int, payload: schemas.RuntimeUpdatePayload):
        async with async_session() as session:
            q = (
                sa.update(models.Game)
                .where(models.Game.id == game_id, models.Game.status == types.GameStatus.active)
