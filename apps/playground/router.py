...
@inject
async def submit(payload: schemas.SubmitPayload, lproblem_id: int, user: User = Depends(get_user),
                 storage: PlaygroundStorage = Depends(Provide[Container.playground_storage])):

    runtime_payload = schemas.RuntimePayload(user_id=user.id,
...
@inject
async def runtimes(lproblem_id: int,
...
@inject
async def runtime(runtime_id: int, user: User = Depends(get_user),
                  storage: PlaygroundStorage = Depends(Provide[Container.playground_storage])):
...
@inject
async def game(game_id: int, user: User = Depends(get_user),
               storage: PlaygroundStorage = Depends(Provide[Container.playground_storage])):
...
@router.get("/games")
@inject
async def games(user_id: int, user: User = Depends(get_user),
                storage: PlaygroundStorage = Depends(Provide[Container.playground_storage])):
